...
**不管是请求还是响应，HTTP 报文的骨架都是同一套：**

```text
┌─────────────────────────────────────┐
│  起始行（请求行 / 状态行）            │  ← 第一行，描述这个报文是干什么的
├─────────────────────────────────────┤
│  头（Headers）                       │  ← 零个或多个键值对，每行一个
│  Host: example.com                   │
│  Content-Type: application/json      │
├─────────────────────────────────────┤
│  空行（CRLF）                         │  ← 固定有一个空行，分隔头和体
├─────────────────────────────────────┤
│  体（Body）                           │  ← 可选，不一定有
│  {"name": "Alice"}                   │
└─────────────────────────────────────┘
...
Content-Length: 45
                                        ← 空行（必不可少！）
{"id": 1, "name": "Alice", "age": 25}   ← 体（204 No Content 没有体）
```
...
打开你项目里的 `统一成功响应格式.md`，对照 HTTP 报文结构看看：

### ① HTTP 状态码 ≠ 业务 code

```python
# HTTP 基础.md 讲的：服务器通过状态码告诉浏览器结果
return JSONResponse(
    status_code=200,                    # ← 这是 HTTP 状态码（状态行里那个）
    content={"code": 200, ...}          # ← 这是你业务 code（在响应体里）
)
```
...
```text
HTTP/1.1 200 OK                        ← 这是 HTTP 状态码
Content-Type: application/json
                                        ← 空行
{"code": 200, "message": "success"}     ← 这是业务 code（在 body 里）
```

| | 位置 | 谁看 | 含义 |
|--|------|------|------|
| HTTP 状态码 | 响应状态行 | 浏览器、Nginx、网关 | 这请求成功了没？（200/404/500） |
| 业务 code | 响应 body 里 | 前端 JS 代码 | 业务上成功了没？（比如 200 但 code 可能是 40001 表示密码错误） |

**所以你的 `success_response` 才要写 `status_code=200` + body 里也塞一个 `code=200`** —— HTTP 告诉浏览器「请求成功了」，body 里的 code 告诉前端「业务也成功了」。如果前端收到 `HTTP 200` 但 body 里的 `code=40001`，就知道「HTTP 连接正常，但业务上密码错了」。
...
```text
HTTP/1.1 200 OK                              ← 状态行（status_code）
Content-Type: application/json               ← 响应头（JSONResponse 自动设置）
Content-Length: 123

{"code": 200, "message": "注册成功", "data": {...}}  ← 响应体
```

`success_response` 做的其实就是**组装这个 body 部分 + 帮你设置好 Content-Type 头**。

### ③ `raise HTTPException(status_code=404)` 等价于什么？
...
**这就是为什么 `HTTP基础.md` 要讲状态码** —— 你的每个 `raise HTTPException` 都在生成 HTTP 响应报文里的状态行和状态码。
